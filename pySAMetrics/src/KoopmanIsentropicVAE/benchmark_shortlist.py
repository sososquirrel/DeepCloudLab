...
def train_one(model, kind, train_loader, val_loader, *, num_epochs=200, epoch_switch=30,
    ...
    for epoch in range(num_epochs):
        ...
        for x_t, x_future in train_loader:
            ...
            # KL only for VAE/KVAE and only if mu/logvar exist
            if is_vae and (mu_t is not None) and (logvar_t is not None):
                # use your helper to keep consistency
                total, _, kl_div = vae_loss(recon_x, x_t, mu_t, logvar_t, beta=1) 
            else:
            ...
            koop_diag, _ = multi_koopman_loss(base_code, fut_codes, steps, beta=1)  # avg over steps
        ...
        with torch.no_grad():
            for x_t, x_future in val_loader:
                ...
                if is_vae and (mu_t is not None) and (logvar_t is not None):
                    _, _, kl_div = vae_loss(recon_x, x_t, mu_t, logvar_t, beta=1)
                else:
                    kl_div = torch.tensor(0.0, device=device)
                ...
                for i in range(x_future.shape[1]):
                    ...
                    fut_codes.append(mu_f if (mu_f is not None) else code_f)
                base_code = mu_t if (mu_t is not None) else code_t
                koop_diag, _ = multi_koopman_loss(base_code, fut_codes, steps, beta=1)
...
def vae_loss(recon_x, x, mu, logvar, beta=1):
    ...
    return total_loss, recon_loss, beta*kl_div
...
# ------------- Main grid -------------
def run_benchmark(data_path, out_dir, dims=(2,4,8,12,16,32), kinds=("ae", "sae", "vae", "ae_koop", "sae_koop", "kvae"),
    ...
    beta_sched = np.clip(np.linspace(0.01, 0.5, 400), 0, 1.0)
...
if __name__ == "__main__":
    run_benchmark(
        ...
        dims=(4,8, 16),#(4,8,16),
        kinds=("vae", "kvae"), #("ae", "sae", "ae_koop", "sae_koop"), #
        num_epochs=200,
        epoch_switch=30,
        steps=(1, 3, 5, 10),
        seed=42,
