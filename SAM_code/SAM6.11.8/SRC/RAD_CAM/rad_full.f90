subroutine rad_full()
  ...
  use slm_vars, only: albedovis_v,albedonir_v,albedovis_s,albedonir_s, &
                      phi_1,phi_2,LAI,landtype,soilw
  ...
  !bloss: Check to see if the domain top is in the mid-troposphere.
  !   This is not ideal, so stop and warn the user.
  if(presi(nz).gt.200.) then
    if(masterproc) then
      write(*,*) 'Warning: This run uses interactive radiation but the domain top is in the troposphere.'
      write(*,*) '  This is not ideal.  If you wish to run with such a domain, consider using RAD_RRTM4PBL.'
      write(*,*) '  Model stopping...  You can run with this setup by commenting out the check at the'
      write(*,*) '  top of SRC/RAD_CAM/rad_full.f90.'
    end if
    call task_abort()
  end if
...
!bloss: Output argument                 coszrs_in(1) = coszrs
	      ...
	      coszrs = coszrs_in(1) ! needed for the isccp simulator
              ...
              call albedo(1,1,landmask(i,j),landtype(i,j),coszrs_in(1), &
                 real(albedovis_v(i,j)),real(albedonir_v(i,j)), &
                 real(albedovis_s(i,j)),real(albedonir_s(i,j)), real(soilw(1,i,j)), &
                 real(phi_1(i,j)),real(phi_2(i,j)),real(LAI(i,j)),asdir(1),aldir(1),asdif(1),aldif(1))
