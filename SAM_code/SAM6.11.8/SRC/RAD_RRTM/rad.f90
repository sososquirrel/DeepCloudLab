...
! dummy variables added for compilerability with SLM
        real:: lwdsxy  (nx, ny)=0.
        real:: swdsxy  (nx, ny)=0.
 ...
 public :: swdsvisxy,swdsnirxy,swdsvisdxy,swdsnirdxy,coszrsxy,lwdsxy,swdsxy
  ...
  subroutine rad_driver 
    ...
    if(presi(nz).gt.200.) then
      if(masterproc) then
        ...
        write(*,*) '  top of SRC/RAD_RRTM/rad.f90.'
      end if
      call task_abort()
    end if
    ...
    if(SLM) then
       print*,'SLM cannot be used with RRTM radiation. STOP...'
       call task_abort()
    end if
    ! ----------------------------------------------------------------------------
